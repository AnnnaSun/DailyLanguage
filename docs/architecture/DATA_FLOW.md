...
| PracticeSession Lifecycle | `POST /api/language-profiles/{languageProfileId}/learning-tasks/{taskId}/practice-sessions`；response / completion `PUT` APIs | `PracticeSessionApplicationService.start / submit / complete` | owned Task + exact material → unique `IN_PROGRESS` Session；Session-row-first response arbitration；Java deterministic assessment；Session / assessment / Task atomic completion + durable replay | `PracticeSessionRepository`, `PracticeSessionMapper` | PostgreSQL `practice_session`, `practice_response`, `deterministic_assessment`, `deterministic_step_assessment`（Flyway V9–V10） | `PracticeSessionTests`, `DeterministicAssessmentTests`, `DeterministicTextAssessmentPolicyTests`, `PracticeSessionApplicationServiceTests`, `PracticeSessionControllerTests`, `PracticeSessionPersistenceIntegrationTests` |
| Owner-scoped Grounded Evaluation Input Read | `GroundedEvaluationInputReader.readOwned`（M1-S8A） | `GroundedEvaluationInputReader` | caller identity 只取自 `UserContext` → owned completed Session → Task gate → assessment / responses → exact-identity material → snapshot 一致性 → `Ready / NotFound / NotCompleted / MaterialUnavailable / InconsistentSnapshot` | 复用 `PracticeSessionRepository.findOwned / findOwnedAssessment / findOwnedResponses` 与 `LearningTaskRepository.findOwned`；短 readOnly transaction，零写入、无 FOR UPDATE | PostgreSQL 既有 `learning_task / practice_session / practice_response / deterministic_assessment` 行（无新 schema）；classpath catalog 按 exact identity 解析（HISTORICAL_ONLY 可读） | `GroundedEvaluationInputReaderTests` 18/18；Reader integration 5/5 + S7 integration 3/3；PostgreSQL 18.6 empty schema Flyway V1–V10 10/10 PASS |
