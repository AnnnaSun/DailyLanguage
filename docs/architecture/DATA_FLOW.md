...
M1 当前实现中，每条 `practice_response` 以 `(sessionId, stepId)` 标识，保存首次接受的 exact learner text，
并同行保存回答时的四类 `ResponseSupportCondition`：demonstration、explanation、hint、responseFrame。
每类 exposure 使用 `UNKNOWN / NOT_PROVIDED / PROVIDED / OPENED`；migration 前历史与当前 HTTP submit
均显式为 `UNKNOWN`。这些值是可观察交互条件，不是理解、正确性、独立性或掌握结论。replay/conflict 不覆盖
首次 snapshot；Grounded Evaluator Reader 只把它作为 trusted input 继续传递，M1 不据此修改长期状态。
...
| Owner-scoped LearningTask Planning | `POST /api/language-profiles/{languageProfileId}/learning-tasks` | `LearningTaskPlanningService` | owned Profile → `PlanningRequest` → deterministic `LearningTaskPlan`；Profile identity guard | `LearningTaskRepository`, `LearningTaskMapper` | PostgreSQL `learning_task`（Flyway V8） | `LearningTaskPlanningControllerTests`, `LearningTaskPlanningServiceTests`, `LearningTaskPlanningIntegrationTests` |
| LearningTask Persistence | `LearningTaskRepository.createOwned / findOwned / tryStart / tryComplete` | M1-S4 create 由 `LearningTaskPlanningService` 接入；M1-S5 start 由 `PracticeSessionApplicationService` 接入 | `LearningTaskPlan` → durable `LearningTask`；trusted owner + Profile + target language validation；`PLANNED → STARTED` 与 Session insert 原子提交 | `LearningTaskRepository`, `LearningTaskMapper` | PostgreSQL `learning_task`（Flyway V8） | `LearningTaskTests`, `LearningTaskPersistenceIntegrationTests`, `PracticeSessionPersistenceIntegrationTests` |
| PracticeSession Lifecycle | `POST /api/language-profiles/{languageProfileId}/learning-tasks/{taskId}/practice-sessions`；response / completion `PUT` APIs | `PracticeSessionApplicationService.start / submit / complete` | owned Task + exact material → safe guided projection + unique `IN_PROGRESS` Session；Session-row-first response arbitration；首次 text 与四类 support exposure 原子持久化；Java deterministic assessment；Session / assessment / Task atomic completion + durable replay | `PracticeSessionRepository`, `PracticeSessionMapper` | PostgreSQL `practice_session`, `practice_response`, `deterministic_assessment`, `deterministic_step_assessment`（Flyway V9–V10、V14） | `PracticeSessionTests`, `DeterministicAssessmentTests`, `DeterministicTextAssessmentPolicyTests`, `PracticeSessionApplicationServiceTests`, `PracticeSessionControllerTests`, `PracticeSessionPersistenceIntegrationTests`；S8T affected integration 35/35 PASS |
| Owner-scoped Grounded Evaluation Input Read | `GroundedEvaluationInputReader.readOwned`（M1-S8A + S8T-B compatibility） | `GroundedEvaluationInputReader` | caller identity 只取自 `UserContext` → owned completed Session → Task gate → assessment / responses（含 support condition）→ exact-identity material → snapshot 一致性 → `Ready / NotFound / NotCompleted / MaterialUnavailable / InconsistentSnapshot` | 复用 `PracticeSessionRepository.findOwned / findOwnedAssessment / findOwnedResponses` 与 `LearningTaskRepository.findOwned`；短 readOnly transaction，零写入、无 FOR UPDATE | PostgreSQL `learning_task / practice_session / practice_response / deterministic_assessment`；classpath catalog 按 exact identity 解析（`HISTORICAL_ONLY` 可读） | `GroundedEvaluationInputReaderTests` 18/18；2026-09-09 S8T affected Reader integration 5/5、Planner/Practice regression 42/42；empty schema Flyway V1–V14 PASS |
| EvaluationRun Creation | `EvaluationRunCreationService.createForReadyInput`（M1-S8B，COMPLETE `2d46df6`） | `EvaluationRunCreationService` | Ready + `UserContext` identity 校验 → Session 行锁（`FOR UPDATE OF session`）→ completion / task 再检查 → 既有 Run 查询（关联 Job 缺失或创建期 identity 不一致 → `IllegalStateException` 不变量损坏 fail closed）→ `SELECT uuidv7()` → 创建 EVALUATION Job → owner-scoped INSERT Run（DB gate JOIN Job 核对完整创建期 identity）→ `Created / Existing / NotFound / NotCompleted / InconsistentInput`；任何 Model dispatch 之前完成 | `PracticeSessionRepository.findOwnedForUpdate`, `EvaluationRunRepository.nextRunId / findOwnedBySessionId / insertOwned`, `ModelCallJobRepository.create`（加入调用方事务）；REQUIRES_NEW 独立事务，任一步失败整体回滚、零 orphan | PostgreSQL `evaluation_run`（Flyway V11 建立 initial PENDING lifecycle；V12 由 S8C 扩展 terminal outcome）与 `model_call_job`；不持久化 Prompt / Rubric / request / Credential | `EvaluationRunCreationServiceTests` 14/14；`EvaluationRunCreationIntegrationTests` 8/8（含并发、回滚、零 orphan、insert gate 六类 Job identity 偏差拒绝）；Codex external verification PASS：PostgreSQL 18.6 empty schema Flyway V1–V11、S8B 8/8、affected ModelCallJob 103/103，0 failures / 0 errors / 0 skipped，临时数据库已删除 |
| Evaluation Model Dispatch | `EvaluationDispatchService.dispatchForReadyInput`（M1-S8D） | `EvaluationTextRequestFactory`, `EvaluationRunCreationService`, `TextGenerationJobDispatch` | Ready → version 0 rubric/prompt v1 → provider-neutral EVALUATION JSON request → UTC now + 7d expiry → S8B durable Run/Job creation → only Created dispatch；Existing 只返回 durable state；异步 Worker → Model Gateway → durable result | 外层 `NEVER`；S8B `REQUIRES_NEW` 返回后才进入同为 `NEVER` 的 existing-Job dispatch；capacity rejection CAS，unknown submission exception 不猜测补偿 | PostgreSQL Run/Job/result；classpath prompt/rubric；request 与 Credential 只在内存；不新增 schema/API，不修改长期 learner state | S8D targeted 29/29；PostgreSQL 18.6 empty schema Flyway V1–V12、dispatch integration 1/1；affected reports 243/0/0；full server 700/0/0（159 conditional skips）；Critical Review PASS |
| Evaluation API Orchestration | `PUT /api/language-profiles/{languageProfileId}/practice-sessions/{sessionId}/evaluation[ /reconciliation]`（M1-S8E-API） | `EvaluationController`, `PracticeSessionEvaluationService` | authenticated + CSRF → owner-scoped Ready read；trigger 验证 `providerId` / transient Credential / fixed EVALUATION route → S8D dispatch → S8E-R consume；reconciliation 无 Credential / route / dispatch，只推进或 replay 同一 Run；Pending → `202 + Location`，durable SUCCEEDED/FAILED → `200` | orchestration 两入口均 `NEVER`；Reader readOnly、Run/Job creation `REQUIRES_NEW`、dispatch transaction-free、consumption read-write transaction 保持分离；owner/profile gate 发生在读取 learner text 与处理 Credential 前 | PostgreSQL Run/Job/result/candidate；Credential、Prompt/raw output 不进 response 或新 persistence；不创建长期 Evidence，不修改 Memory/Weakness/Level/Mastery | Critical Review PASS；local targeted 43 executed + 2 conditional skips；full server 727/0/0（162 conditional skips）；PostgreSQL 18.6 empty schema Flyway V1–V13 13/13、S8 evaluator integration 31/31，临时数据库已删除；live Provider NOT_RUN |
| Grounded Evaluator Validation | `SemanticGroundingValidator.validate` | S7 module-local validator；S8C `EvaluationResultConsumptionService` 使用该 validator 消费 durable Model result | untrusted JSON + trusted completed snapshot → token guard / strict binding → input consistency → rubric → learner-text occurrence / Java offsets → whole-batch `Validated / Rejected` | Validator 本身零持久化；S8C 只接收 typed grounding result | classpath English rubric；只读传入的 durable snapshot | `SemanticGroundingValidatorTests`, `ClasspathRubricSourceTests`, `SemanticGroundingIntegrationTests` |
| Evaluation Result Consumption / Reconciliation | `EvaluationResultConsumptionService.consumeForReadyInput`（M1-S8C / S8E-R / S8E-API） | `EvaluationResultConsumptionService` | authenticated identity → owner/profile-scoped Run row lock → Ready/Run/Job full identity gate；current success → durable result + Java grounding + Job consume + candidate/rejection + Run terminal；terminal Model failure → `MODEL_CALL_FAILED`；old/expired/stale/depleted result → `MODEL_RESULT_UNAVAILABLE`；`CREATED / RUNNING` 返回 `Pending(run)`；terminal replay 不重新 grounding | `EvaluationRunRepository.findOwnedBySessionIdForUpdate / insertValidatedCandidate / insertValidatedClaim / tryFinalizeOwned`；`ModelCallJobRepository.findTextGenerationResultByJobIdAndUserId / tryConsumeSucceededResult / tryMarkSucceededResultStale / tryExpireSucceededResult`；单一 read-write transaction，本次 Job mutation（如有）与 Run outcome 原子提交；S8E-API 直接投影锁内 durable Pending snapshot，不做第二次查询；无法解释的 CAS race fail closed | PostgreSQL `evaluation_run`, `validated_semantic_candidate`, `validated_semantic_claim`（Flyway V12–V13）与既有 ModelCallJob/result tables；PostgreSQL time/constraints 是 expiry/outcome authority；不保存 Credential、完整 Prompt 或 rejected raw output | S8E-R targeted 35/35、full server 703/0/0（160 conditional skips）、external 28/28；S8E-API local full server 727/0/0（162 conditional skips）、PostgreSQL 18.6 S8 evaluator integration 31/31；V12→V13 backfill PASS；Critical Review / Architecture PASS |
