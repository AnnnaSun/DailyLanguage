...
| Evaluation Result Consumption | `EvaluationResultConsumptionService.consumeForReadyInput`（M1-S8C） | `EvaluationResultConsumptionService` | authenticated identity → owner/profile-scoped Run row lock → Ready/Run/Job full identity gate → durable raw result → Java grounding → Job `NOT_READY → CONSUMED` CAS → candidate + claims 或 safe rejection → Run `PENDING → SUCCEEDED | FAILED` CAS；terminal replay 不重新 grounding | `EvaluationRunRepository.findOwnedBySessionIdForUpdate / insertValidatedCandidate / insertValidatedClaim / tryFinalizeOwned`, `ModelCallJobRepository.findTextGenerationResultByJobIdAndUserId / tryConsumeSucceededResult`；单一 read-write transaction，任何 outcome 写失败连同 Job consumption 回滚 | PostgreSQL `evaluation_run`, `validated_semantic_candidate`, `validated_semantic_claim`（Flyway V12）与既有 ModelCallJob/result tables；不保存 Credential、完整 Prompt 或 rejected raw output | `EvaluationResultConsumptionServiceTests` 19/19；`EvaluationResultConsumptionIntegrationTests` 12/12；affected unit 106/106、integration 43/43；fresh PostgreSQL 18.6 Flyway V1–V12 12/12；Critical Review PASS |
