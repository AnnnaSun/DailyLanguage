...
| PracticeSession Lifecycle | `POST /api/language-profiles/{languageProfileId}/learning-tasks/{taskId}/practice-sessions`；`PUT /api/language-profiles/{languageProfileId}/practice-sessions/{sessionId}/responses/{stepId}` | `PracticeSessionApplicationService.start / submit` | owned Task + exact material → unique `IN_PROGRESS` Session；owned Session row lock + exact step → first response / replay / conflict | `PracticeSessionRepository`, `PracticeSessionMapper` | PostgreSQL `practice_session`, `practice_response`（Flyway V9） | `PracticeSessionTests`, `PracticeSessionApplicationServiceTests`, `PracticeSessionControllerTests`, `PracticeSessionPersistenceIntegrationTests` |
