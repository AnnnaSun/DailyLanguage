...
| Owner-scoped Grounded Evaluation Input Read | `GroundedEvaluationInputReader.readOwned`（M1-S8A，READY_TO_COMMIT） | `GroundedEvaluationInputReader` | caller identity 只取自 `UserContext` → owned completed Session → Task gate → assessment / responses → exact-identity material → snapshot 一致性 → `Ready / NotFound / NotCompleted / MaterialUnavailable / InconsistentSnapshot` | 复用 `PracticeSessionRepository.findOwned / findOwnedAssessment / findOwnedResponses` 与 `LearningTaskRepository.findOwned`；短 readOnly transaction，零写入、无 FOR UPDATE | PostgreSQL 既有 `learning_task / practice_session / practice_response / deterministic_assessment` 行（无新 schema）；classpath catalog 按 exact identity 解析（HISTORICAL_ONLY 可读） | `GroundedEvaluationInputReaderTests` 18/18；Reader integration 5/5 + S7 integration 3/3；PostgreSQL 18.6 empty schema Flyway V1–V10 10/10 PASS |
| Grounded Evaluator Validation | `SemanticGroundingValidator.validate` | N/A — M1-S7 module-local validator；trusted input 由 M1-S8A `GroundedEvaluationInputReader` 产生，ModelCallJob assembler（S8B+）未实现 | untrusted JSON + trusted completed snapshot → token guard / strict binding → input consistency → rubric → learner-text occurrence / Java offsets → whole-batch `Validated / Rejected` | N/A — S7 不持久化 | classpath English rubric；只读传入的 durable snapshot | `SemanticGroundingValidatorTests`, `ClasspathRubricSourceTests`, `SemanticGroundingIntegrationTests` |
