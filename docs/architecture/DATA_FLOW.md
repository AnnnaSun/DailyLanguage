...
| Evaluation Result Consumption / Reconciliation | `EvaluationResultConsumptionService.consumeForReadyInput`（M1-S8C / S8E-R） | `EvaluationResultConsumptionService` | authenticated identity → owner/profile-scoped Run row lock → Ready/Run/Job full identity gate；current success → durable result + Java grounding + Job consume + candidate/rejection + Run terminal；terminal Model failure → `MODEL_CALL_FAILED`；old/expired/stale/depleted result → `MODEL_RESULT_UNAVAILABLE`；`CREATED / RUNNING` 保持 Pending；terminal replay 不重新 grounding | `EvaluationRunRepository.findOwnedBySessionIdForUpdate / insertValidatedCandidate / insertValidatedClaim / tryFinalizeOwned`；`ModelCallJobRepository.findTextGenerationResultByJobIdAndUserId / tryConsumeSucceededResult / tryMarkSucceededResultStale / tryExpireSucceededResult`；单一 read-write transaction，本次 Job mutation（如有）与 Run outcome 原子提交；已 terminal Job 是 durable 前置事实；无法解释的 CAS race fail closed | PostgreSQL `evaluation_run`, `validated_semantic_candidate`, `validated_semantic_claim`（Flyway V12–V13）与既有 ModelCallJob/result tables；PostgreSQL time/constraints 是 expiry/outcome authority；不保存 Credential、完整 Prompt 或 rejected raw output | S8E-R targeted 35/35、implementation-stage full server 703/0/0（160 conditional skips）；PostgreSQL 18.6 empty-schema Flyway V1–V13、相关 integration 28/28、V12→V13 backfill PASS；Critical Review / Architecture PASS |
