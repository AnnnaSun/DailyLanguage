...
| Planner | `server/src/main/java/com/dailylanguage/planner` | `LearningTaskPlanner`, `DeterministicLearningTaskPlanner` | `PlanningRequest`, `PlanningResult`, `LearningTaskPlan` | `DeterministicLearningTaskPlannerTests` | PARTIAL — M1-S2 deterministic core COMPLETE (`fcefedb` + review fixes `4322499`)；Java 从 exact language-pair Catalog candidates 执行 hard filtering、stable fallback 与 final identity/language/scaffold validation，返回未持久化 `LearningTaskPlan` 或 typed `Unavailable`；无 Model enrichment、PostgreSQL persistence、owner-scoped API、Session 或 learner-state mutation |
