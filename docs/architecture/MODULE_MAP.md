...
| Planner | `server/src/main/java/com/dailylanguage/planner`, `server/src/main/resources/mapper/LearningTaskMapper.xml`, `server/src/main/resources/db/migration/V8__add_learning_task.sql` | `LearningTaskPlanner`, `DeterministicLearningTaskPlanner`, `LearningTaskRepository` | `PlanningRequest`, `PlanningResult`, `LearningTaskPlan`, `LearningTask`, `LearningTaskMapper` | `DeterministicLearningTaskPlannerTests`, `LearningTaskTests`, `LearningTaskPersistenceIntegrationTests` | PARTIAL — M1-S2 deterministic core COMPLETE；M1-S3 persistence candidate REVIEWED / VERIFIED / UNCOMMITTED：Java Planner 返回 `LearningTaskPlan`，Repository 使用 trusted owner + Profile + target language 原子创建 UUIDv7 durable task，并提供 owner/profile-scoped read 与 `PLANNED → STARTED → COMPLETED` conditional transition；PostgreSQL 是 identity/status/timestamp authority；无 owner-scoped API、Session、Model enrichment 或 learner-state mutation |
