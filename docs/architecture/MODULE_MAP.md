...
（V1 范围、Phase 分配及 S8T-A/B/C Contract 已批准）。S8T 已在 Content 实现 typed guided material，
由 Planner 选择 exact published version，Practice 下发教学投影并保存 support-condition snapshot，Evaluator
trusted input 读取该 snapshot；未新增顶层 Learning module。M2 才执行 qualification / aggregation，S11 才记录
真实 UI 暴露，Review 后续安排再次出现。
