...
| Evaluator | `server/src/main/java/com/dailylanguage/evaluator`, `server/src/main/resources/evaluator/rubrics` | `SemanticGroundingValidator.validate`; `GroundedEvaluationInputReader.readOwned` | `GroundedEvaluationInput`, `SemanticEvaluationOutput`, `SemanticEvaluationRubric`, `SemanticGroundingResult`, `ValidatedSemanticCandidate`, `GroundedClaim`, `RubricSource`, `ClasspathRubricSource`, `GroundedEvaluationInputResult` | `SemanticGroundingValidatorTests`, `ClasspathRubricSourceTests`, `SemanticGroundingIntegrationTests`, `GroundedEvaluationInputReaderTests`, `GroundedEvaluationInputReaderIntegrationTests` | PARTIAL — M1-S7 Grounded Evaluator contract implemented：原始 JSON token type guard + strict record binding；completed Task/Session/assessment/material/response-set consistency；learner-text literal occurrence、UTF-16 offsets、surrogate boundary 与 versioned rubric issue allowlist；任一 claim 失败整批拒绝，零持久化与长期 learner-state mutation。Rubric read seam 与 classpath adapter 已分离为独立 source files。M1-S8A owner-scoped read entry（READY_TO_COMMIT）已实现：`readOwned` 在短 readOnly transaction 内经 owner-scoped Repository 读取 completed Task/Session/assessment/responses 并按 exact material identity 解析（HISTORICAL_ONLY 可读），返回 `Ready / NotFound / NotCompleted / MaterialUnavailable / InconsistentSnapshot`；ownership 失败先于 private reads、零写入、无 FOR UPDATE；Critical / delta Review PASS，Reader integration 5/5 + S7 integration 3/3、PostgreSQL 18.6 empty schema Flyway V1–V10 10/10 PASS。无 ModelCallJob、EvaluationRun、candidate persistence、HTTP API、Japanese rubric 或 M2 qualification |
...
| Persistence Infrastructure | `server/src/main/resources/db`, `server/src/main/resources/mapper`, `server/src/main/java/com/dailylanguage/persistence` | Flyway, MyBatis Mapper XML | PostgreSQL UUID TypeHandler, parameterized Mapper statements, auth identity / credential schema, Model Call Job tables, LearningTask、PracticeSession、PracticeResponse 与 DeterministicAssessment schema | `PersistenceIdentityIntegrationTests`, `LocalAuthenticationPersistenceIntegrationTests`, `MapperSqlSafetyTests`, ModelCallJob integration tests, `LearningTaskPersistenceIntegrationTests`, `LearningTaskPlanningIntegrationTests`, `PracticeSessionPersistenceIntegrationTests`, `SemanticGroundingIntegrationTests` | PARTIAL — identity、local credential、Model Call Job、LearningTask、PracticeSession 与 DeterministicAssessment persistence foundation；M1-S7 fresh disposable PostgreSQL 18.6 empty database 已验证 Flyway V1–V10 10/10、affected integration 50/50 与 wider server regression 622 tests / 0 failures / 0 errors / 11 Redis 相关条件跳过；Grounded Evaluator 本身不新增 schema 或写入 |
