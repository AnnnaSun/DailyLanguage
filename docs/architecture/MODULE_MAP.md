...
| Evaluator | `server/src/main/java/com/dailylanguage/evaluator`, `server/src/main/resources/evaluator/rubrics` | `SemanticGroundingValidator.validate` | `GroundedEvaluationInput`, `SemanticEvaluationOutput`, `SemanticEvaluationRubric`, `SemanticGroundingResult`, `ValidatedSemanticCandidate`, `GroundedClaim`, `RubricSource`, `ClasspathRubricSource` | `SemanticGroundingValidatorTests`, `ClasspathRubricSourceTests`, `SemanticGroundingIntegrationTests` | PARTIAL — M1-S7 Grounded Evaluator contract implemented：原始 JSON token type guard + strict record binding；completed Task/Session/assessment/material/response-set consistency；learner-text literal occurrence、UTF-16 offsets、surrogate boundary 与 versioned rubric issue allowlist；任一 claim 失败整批拒绝，零持久化与长期 learner-state mutation。Rubric read seam 与 classpath adapter 已分离为独立 source files；无 ModelCallJob、EvaluationRun、candidate persistence、HTTP API、Japanese rubric 或 M2 qualification |
