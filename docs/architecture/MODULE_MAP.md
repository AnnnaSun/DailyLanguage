...
| Practice Runtime | `server/src/main/java/com/dailylanguage/practice`, `server/src/main/resources/mapper/PracticeSessionMapper.xml`, `server/src/main/resources/db/migration/V9__add_practice_session.sql` | `PracticeSessionController`, `PracticeSessionApplicationService`, `PracticeSessionRepository` | `PracticeSession`, `LearnerResponse`, `StartResult`, `SubmitResult`, `PracticeMaterialView`, `PracticeSessionMapper` | `PracticeSessionControllerTests`, `PracticeSessionApplicationServiceTests`, `PracticeSessionTests`, `PracticeSessionPersistenceIntegrationTests` | PARTIAL — M1-S5 PracticeSession lifecycle COMPLETE (`b6cde9d`)：authenticated `UserContext` → owned Task / exact material → atomic `PLANNED → STARTED` + unique `IN_PROGRESS` Session；response 以 Session row lock、owner/profile/status SQL gate 与 `(sessionId, stepId)` PK 裁决首次接受、exact replay 或 conflict；无 completion / abandon API、assessment、Evaluator、Evidence 或长期 learner-state mutation |
