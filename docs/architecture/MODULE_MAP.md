...
教学流程设计增量见 [`GUIDED_LANGUAGE_LEARNING.md`](../features/GUIDED_LANGUAGE_LEARNING.md)
（V1 范围与 Phase 分配已批准，Current Slice Contract 未批准，尚未实现）。Content 承载示范与解释，
Planner 选择学习需要，Practice 承载输入、理解与使用交互，Evaluator / Memory 区分辅助条件下的表现，
Review 安排后续再次出现。复用下列模块，不新增顶层 Learning module；本说明不代表 Physical Map 已实现。
首次学习的简短语法说明属于教学支架，区别于重复问题触发的 Grammar Repair。
...
| Practice Runtime | `server/src/main/java/com/dailylanguage/practice`, `server/src/main/resources/mapper/PracticeSessionMapper.xml`, `server/src/main/resources/db/migration/V9__add_practice_session.sql`, `server/src/main/resources/db/migration/V10__add_deterministic_assessment.sql` | `PracticeSessionController`, `PracticeSessionApplicationService`, `PracticeSessionRepository` | `PracticeSession`, `LearnerResponse`, `DeterministicAssessment`, `DeterministicTextAssessmentPolicy`, `StartResult`, `SubmitResult`, `CompletionResult`, `PracticeMaterialView`, `PracticeSessionMapper` | `PracticeSessionControllerTests`, `PracticeSessionApplicationServiceTests`, `PracticeSessionTests`, `DeterministicAssessmentTests`, `DeterministicTextAssessmentPolicyTests`, `PracticeSessionPersistenceIntegrationTests` | PARTIAL — M1-S5 lifecycle COMPLETE (`b6cde9d`) + M1-S6 deterministic completion COMPLETE (`82aced2`)：authenticated `UserContext` → owned Task / exact material → atomic Session lifecycle；response 与 completion 共用 Session-row-first lock；Java `M1_TEXT_EXACT_V1` 计算 step result，Session / assessment / Task completion 单事务持久化并支持 durable replay；无 abandon、semantic Model call、Evidence 或长期 learner-state mutation |
| Conversation | TBD | TBD | TBD | TBD | NOT_STARTED |
| Reading | TBD | TBD | TBD | TBD | NOT_STARTED |
| Vocabulary | TBD | TBD | TBD | TBD | NOT_STARTED |
| Evaluator | `server/src/main/java/com/dailylanguage/evaluator`, `server/src/main/resources/evaluator/rubrics`, `server/src/main/resources/evaluator/prompts`, `server/src/main/resources/mapper/EvaluationRunMapper.xml`, `server/src/main/resources/db/migration/V11__add_evaluation_run.sql`, `server/src/main/resources/db/migration/V12__add_evaluation_run_outcome.sql`, `server/src/main/resources/db/migration/V13__add_evaluation_run_failure_reason.sql` | `SemanticGroundingValidator.validate`; `GroundedEvaluationInputReader.readOwned`; `EvaluationRunCreationService.createForReadyInput`; `EvaluationDispatchService.dispatchForReadyInput`; `EvaluationTextRequestFactory.build`; `EvaluationResultConsumptionService.consumeForReadyInput` | `GroundedEvaluationInput`, `SemanticGroundingResult`, `ValidatedSemanticCandidate`, `GroundedClaim`, `GroundedEvaluationInputResult`, `EvaluationRun`, `EvaluationRun.FailureReason`, creation/dispatch/consumption typed results, `EvaluationRunRepository`, `EvaluationRunMapper` | Grounding、Reader、Run creation、request/dispatch 与 result consumption/reconciliation unit/integration tests | PARTIAL — M1-S7 grounding、S8A owner-scoped read、S8B atomic Run/Job creation、S8C durable result consumption 和 S8D versioned request/transient dispatch 已提交至 `8228d64`。S8E-R 增加 workflow-owned reconciliation：terminal Model failure → `MODEL_CALL_FAILED`，expired/stale/depleted result → `MODEL_RESULT_UNAVAILABLE`；旧 workflow 在 grounding 前 stale，expiry 由 PostgreSQL time/CAS 裁决，terminal replay 不重新 grounding。S8E-R Critical Review / Architecture PASS；targeted 35/35、implementation-stage full server 703/0/0（160 conditional skips）；PostgreSQL 18.6 empty-schema Flyway V1–V13、相关 integration 28/28、V12→V13 backfill PASS。无 HTTP API、scheduler、automatic retry、Japanese rubric 或 M2 qualification |
| Learning Memory | TBD | TBD | TBD | TBD | NOT_STARTED |
| Weakness / Skill | TBD | TBD | TBD | TBD | NOT_STARTED |
| Review | TBD | TBD | TBD | TBD | NOT_STARTED |
| Built-in Learning Material Boundary | `server/src/main/java/com/dailylanguage/content`, `server/src/main/resources/content/builtin`, `server/tools/GenerateBuiltInMaterialHash.java` | `LearningMaterialCatalog` | `PublishedLearningMaterial`, `TargetPracticeCore`, `SupportScaffold`, `MaterialSourceLineage`, `MaterialQueryResult`, `BuiltInMaterialPack`, `ClasspathBuiltInMaterialLoader`, `BuiltInLearningMaterialCatalog` | `PublishedLearningMaterialImmutabilityTests`, `ClasspathBuiltInMaterialLoaderTests`, `BuiltInLearningMaterialCatalogTests`, `BuiltInLearningMaterialCatalogContextTests` | PARTIAL — M1-S1 boundary + M1-S2 review fixes：language-pair generic immutable content、manifest SHA-256 lineage、eager fail-closed validation、`PLANNABLE / HISTORICAL_ONLY` version separation 已实现；Deterministic Planner 消费 list/resolve boundary，M1-S5 Practice Runtime 按 Task 锁定的 exact identity 解析 material 并下发安全 projection；M3 Content pipeline / publish storage 未开始 |
...
| Persistence Infrastructure | `server/src/main/resources/db`, `server/src/main/resources/mapper`, `server/src/main/java/com/dailylanguage/persistence` | Flyway, MyBatis Mapper XML | PostgreSQL UUID TypeHandler, parameterized Mapper statements, auth identity / credential schema, Model Call Job tables, LearningTask、PracticeSession、PracticeResponse、DeterministicAssessment、EvaluationRun 与 validated semantic candidate schema | Persistence、ModelCallJob、LearningTask、PracticeSession、grounding、EvaluationRun creation/result consumption integration tests | PARTIAL — Flyway V11 建立 `evaluation_run`，V12 增加 terminal outcome 与 normalized candidate/claim，V13 增加 closed `failure_reason`、outcome pairing、V12 grounding-failure backfill 与 pending-run partial index。2026-09-08 disposable PostgreSQL 18.6 empty-schema Flyway V1–V13 13/13、相关 integration 28/28；独立 V12→V13 upgrade/backfill 与 constraints/index catalog inspection PASS；临时容器/数据库已删除，primary database 未使用，未执行 Flyway repair 或 checksum 修改 |
