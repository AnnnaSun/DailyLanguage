...
| Model Gateway | `server/src/main/java/com/dailylanguage/modelgateway` | `TextGenerationPort`, `RoutedTextGenerationPort` | `ModelPurpose`, `ModelOperation`, `ModelRouteKey`, `ProviderId`, `ModelId`, `TransientProviderCredential`, `ModelResult`, `ModelFailure`, `ModelFailureKind`, `ModelUsage`, `ModelProviderCallException`, `TextGenerationRequest`, `TextMessage`, `TextOutputSpecification`, `TextGenerationResponse`, `TextGenerationRoute`, `FixedTextGenerationRoutes`, `TextGenerationProviderAdapter`, `OpenAiCompatibleProviderConfig`, `OpenAiCompatibleTextGenerationAdapter` | Existing Model Gateway tests plus `OpenAiCompatibleProviderConfigTests`, `OpenAiCompatibleTextPayloadMapperTests`, `OpenAiCompatibleTextGenerationAdapterTests` | PARTIAL — M0-S7B COMPLETE (`7f5f59f`)；DeepSeek-first OpenAI-compatible non-streaming HTTP Adapter、explicit Credential Bearer header、no-redirect boundary 与 safe failure classification 已实现；无 Spring runtime wiring、Browser / HTTPS ingress、live DeepSeek verification、Structured Output validation 或 Trace persistence |
