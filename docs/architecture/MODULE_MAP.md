...
| Model Gateway | `server/src/main/java/com/dailylanguage/modelgateway` | `TextGenerationPort`（只有 contract，无 implementation） | `ModelPurpose`, `ModelOperation`, `ModelRouteKey`, `ProviderId`, `ModelId`, `ModelResult`, `ModelFailure`, `ModelFailureKind`, `ModelUsage`, `TextGenerationRequest`, `TextMessage`, `TextOutputSpecification`, `TextGenerationResponse` | `ModelRouteKeyTests`, `ProviderModelIdentityTests`, `ModelResultTests`, `ModelFailureTests`, `TextGenerationRequestTests`, `TextGenerationResponseTests`, `TextGenerationPortTests` | PARTIAL — M0-S6C COMPLETE / READY_TO_COMMIT；无 route resolver 或 Provider Adapter |
