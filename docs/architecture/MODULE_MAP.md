...
| Evaluator | `server/src/main/java/com/dailylanguage/evaluator`, `server/src/main/resources/evaluator/rubrics` | `SemanticGroundingValidator.validate`; `GroundedEvaluationInputReader.readOwned`; `EvaluationRunCreationService.createForReadyInput` | `GroundedEvaluationInput`, `SemanticEvaluationOutput`, `SemanticEvaluationRubric`, `SemanticGroundingResult`, `ValidatedSemanticCandidate`, `GroundedClaim`, `RubricSource`, `ClasspathRubricSource`, `GroundedEvaluationInputResult`, `EvaluationRun`, `EvaluationRunCreationService.CreationResult`, `EvaluationRunRepository`, `EvaluationRunMapper` | `SemanticGroundingValidatorTests`, `ClasspathRubricSourceTests`, `SemanticGroundingIntegrationTests`, `GroundedEvaluationInputReaderTests`, `GroundedEvaluationInputReaderIntegrationTests`, `EvaluationRunCreationServiceTests`, `EvaluationRunCreationIntegrationTests` | PARTIAL — M1-S7 Grounded Evaluator contract implemented：原始 JSON token type guard + strict record binding；completed Task/Session/assessment/material/response-set consistency；learner-text literal occurrence、UTF-16 offsets、surrogate boundary 与 versioned rubric issue allowlist；任一 claim 失败整批拒绝。M1-S8A owner-scoped read entry 已实现并提交：`readOwned` 短 readOnly transaction、exact material identity（HISTORICAL_ONLY 可读）、ownership 失败先于 private reads、零写入。M1-S8B EvaluationRun creation（READY_TO_COMMIT）已实现：`createForReadyInput` 在 REQUIRES_NEW 事务内经 Session 行锁串行化，于任何 Model dispatch 前原子创建 `evaluation_run`（Flyway V11，PENDING-only）与唯一 EVALUATION `model_call_job`（workflowId=runId、uuidv7 先取）；重复/并发返回同一 `Existing` 关联，失败整体回滚零 orphan；既有 Run 的关联 Job 缺失或六类创建期 identity（含 operation）不一致以 `IllegalStateException`（不变量损坏）fail closed，数据库 insert gate JOIN Job 双层核对完整创建期 identity；unit 14/14、integration 8/8（临时 PostgreSQL 18.6，含并发/回滚/insert gate 直测）、wider regression 667 tests / 0 failures / 11 Redis skips PASS；Codex 4 review findings 已修复、delta Review PASS；external verification 使用 disposable PostgreSQL 18.6：Flyway V1–V11、S8B 8/8、affected ModelCallJob 103/103 PASS，0 failures / 0 errors / 0 skipped，临时数据库已删除。无 Model dispatch、candidate persistence、HTTP API、Japanese rubric 或 M2 qualification |
...
| Persistence Infrastructure | `server/src/main/resources/db`, `server/src/main/resources/mapper`, `server/src/main/java/com/dailylanguage/persistence` | Flyway, MyBatis Mapper XML | PostgreSQL UUID TypeHandler, parameterized Mapper statements, auth identity / credential schema, Model Call Job tables, LearningTask、PracticeSession、PracticeResponse、DeterministicAssessment 与 EvaluationRun schema | `PersistenceIdentityIntegrationTests`, `LocalAuthenticationPersistenceIntegrationTests`, `MapperSqlSafetyTests`, ModelCallJob integration tests, `LearningTaskPersistenceIntegrationTests`, `LearningTaskPlanningIntegrationTests`, `PracticeSessionPersistenceIntegrationTests`, `SemanticGroundingIntegrationTests`, `EvaluationRunCreationIntegrationTests` | PARTIAL — identity、local credential、Model Call Job、LearningTask、PracticeSession、DeterministicAssessment 与 EvaluationRun persistence foundation；M1-S8B Flyway V11（`evaluation_run`：UNIQUE session/job、PENDING-only、completed_at NULL，ownership 经 session→task 还原）已在本地 docker compose 临时 PostgreSQL 18.6 空库验证 V1–V11 全部应用，S8B integration 8/8 与 Zcode wider server regression 667 tests / 0 failures / 0 errors / 11 Redis 条件跳过 PASS；Codex disposable PostgreSQL 18.6 empty-schema Flyway V1–V11、S8B 8/8、affected ModelCallJob 103/103 PASS，0 failures / 0 errors / 0 skipped，临时数据库已删除 |
