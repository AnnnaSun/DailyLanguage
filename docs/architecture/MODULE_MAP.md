...
| Model Gateway | `server/src/main/java/com/dailylanguage/modelgateway` | N/A — S6A/S6B 只有 portable contract | `ModelPurpose`, `ModelOperation`, `ModelRouteKey`, `ProviderId`, `ModelId`, `ModelResult`, `ModelFailure`, `ModelFailureKind` | `ModelRouteKeyTests`, `ProviderModelIdentityTests`, `ModelResultTests`, `ModelFailureTests` | PARTIAL — M0-S6B COMPLETE；无 invocation Port 或 Provider Adapter |
