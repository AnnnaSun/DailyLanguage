...
| Model Gateway | `server/src/main/java/com/dailylanguage/modelgateway` | `TextGenerationPort`, `RoutedTextGenerationPort`, `TextGenerationGatewayConfiguration`, `ProviderConnectionVerificationService`, `ModelProviderPresetController` | `ModelPurpose`, `ModelOperation`, `ModelRouteKey`, `ProviderId`, `ModelId`, `TransientProviderCredential`, `ModelResult`, `ModelFailure`, `ModelFailureKind`, `ModelUsage`, `ModelProviderCallException`, `TextGenerationRequest`, `TextMessage`, `TextOutputSpecification`, `TextGenerationResponse`, `TextGenerationRoute`, `FixedTextGenerationRoutes`, `TextGenerationProviderAdapter`, `OpenAiCompatibleProviderConfig`, `OpenAiCompatibleTextGenerationAdapter`, `TextGenerationGatewayProperties`, `ProviderPreset` | Existing Model Gateway tests plus `ProviderConnectionVerificationServiceTests`, `ModelProviderPresetControllerTests`, `OpenAiCompatibleProviderConfigTests`, `OpenAiCompatibleTextPayloadMapperTests`, `OpenAiCompatibleTextGenerationAdapterTests`, `TextGenerationGatewayConfigurationTests` | PARTIAL — M0-S7D COMPLETE (`4deed20`)；DeepSeek-first OpenAI-compatible runtime 与 authenticated Backend Credential API verification 已实现；Model Gateway Ownership 保持 L2；无 Hosted TLS evidence、Browser local/session storage UI、live DeepSeek evidence、Structured Output validation 或 Trace persistence |
