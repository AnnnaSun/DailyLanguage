...
| Evaluator | `server/src/main/java/com/dailylanguage/evaluator`, `server/src/main/resources/evaluator/rubrics`, `server/src/main/resources/evaluator/prompts`, `server/src/main/resources/mapper/EvaluationRunMapper.xml`, `server/src/main/resources/db/migration/V11__add_evaluation_run.sql`, `server/src/main/resources/db/migration/V12__add_evaluation_run_outcome.sql` | `SemanticGroundingValidator.validate`; `GroundedEvaluationInputReader.readOwned`; `EvaluationRunCreationService.createForReadyInput`; `EvaluationDispatchService.dispatchForReadyInput`; `EvaluationTextRequestFactory.build`; `EvaluationResultConsumptionService.consumeForReadyInput` | `GroundedEvaluationInput`, `SemanticGroundingResult`, `ValidatedSemanticCandidate`, `GroundedClaim`, `GroundedEvaluationInputResult`, `EvaluationRun`, creation/dispatch/consumption typed results, `EvaluationRunRepository`, `EvaluationRunMapper` | Grounding、Reader、Run creation、request/dispatch 与 result consumption unit/integration tests | PARTIAL — M1-S7 grounding、S8A owner-scoped read、S8B atomic Run/Job creation 和 S8C durable result consumption 已提交至 `de29ada`。M1-S8D 新增 versioned Evaluator prompt/request、固定 EVALUATION route、7d result TTL 与 durable commit 后的 transient dispatch；只有新建 Run/Job 被提交，Existing 不重复调用 Provider，request/Credential 不持久化。S8D Critical Review PASS；fresh targeted 29/29、PostgreSQL 18.6 empty-schema Flyway V1–V12、dispatch integration 1/1、full server 700 tests / 0 failures / 0 errors / 159 conditional skips。无 HTTP API、S8E reconciliation、automatic retry、Japanese rubric 或 M2 qualification |
