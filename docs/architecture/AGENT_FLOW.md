...
| Evaluator | `server/src/main/java/com/dailylanguage/evaluator` | `SemanticGroundingValidator.validate` | N/A — M1-S7 不调用 Model | `GroundedEvaluationInput`：completed Task / Session / assessment / exact material / complete response set；production owner-scoped assembler 属于 M1-S8 | classpath `RubricSource` read boundary；无 Agent Tool | `SemanticEvaluationOutput` → `SemanticGroundingResult.Validated / Rejected` | `SemanticGroundingValidatorTests`, `ClasspathRubricSourceTests`, `SemanticGroundingIntegrationTests` |
