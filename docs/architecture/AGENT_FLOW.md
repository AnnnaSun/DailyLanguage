...
| Evaluator | `server/src/main/java/com/dailylanguage/evaluator` | `GroundedEvaluationInputReader.readOwned`; `EvaluationRunCreationService.createForReadyInput`; `EvaluationResultConsumptionService.consumeForReadyInput`; `SemanticGroundingValidator.validate` | N/A — M1-S8D prompt / dispatch 尚未实现 | owner-scoped completed Task / Session / assessment / exact material / complete response set；Run 绑定 Job 的 durable text result；只接收 authenticated `UserContext` | classpath `RubricSource` read boundary；PostgreSQL-bound Run/Job/candidate repositories，不是 Agent Tool | `GroundedEvaluationInputResult`; durable `EvaluationRun`; `ConsumptionResult` 携带 terminal `Validated / Rejected` outcome | Reader、Run creation、grounding 与 result consumption unit/integration tests；S8C affected unit 106/106、integration 43/43 PASS |
